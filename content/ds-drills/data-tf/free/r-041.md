...
- **`df_receipt`** の売上年⽉⽇ (`sales_ymd`)、顧客ID (`customer_id`)、売上金額 (`amount`)

主要なカラムを抜き出してデータの一部を確認します。

```r {name="R"}
df_receipt %>% select(sales_ymd, customer_id, amount)
```
...
# A tibble: 104,681 × 3
  sales_ymd customer_id    amount
      <int> <chr>           <dbl>
1  20181103 CS006214000001    158
2  20181118 CS008415000097     81
3  20170712 CS028414000014    170
4  20190205 ZZ000000000000     25
5  20180821 CS025415000050     90
6  20190605 CS003515000195    138
7  20181205 CS024514000042     30
8  20190922 CS040415000178    128
9  20170504 ZZ000000000000    770
...
```
...
- **`db_receipt`** の売上年⽉⽇ (`sales_ymd`)、顧客ID (`customer_id`)、売上金額 (`amount`)

主要なカラムを抜き出してデータの一部を確認します。

```r {name="R"}
db_receipt %>% select(sales_ymd, customer_id, amount) %>% collect()
```
...
- **`receipt`** テーブルの売上年⽉⽇ (`sales_ymd`)、顧客ID (`customer_id`)、売上金額 (`amount`)

主要なカラムを抜き出してデータの一部を確認します。

```r {name="R"}
"SELECT sales_ymd, customer_id, amount FROM receipt" %>% db_get_query(con)
```
