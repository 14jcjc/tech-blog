...
はじめに補足しますが、dbplyr の SQL 変換はいつでも完璧というわけではありません。
...
以上を踏まえた上でご覧いただければと思います。

本題に入りますが、`tbl()` で作成したテーブルの参照 `db_sales` は次の `SELECT` 文を生成します。
...
すべての dplyr 操作はこのような `SELECT` 文を元にして SQL に変換されます。
