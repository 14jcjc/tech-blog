...
`df_receipt` データフレームを使用して、日ごとの売上合計を計算し、前回との差分を求めます。
...
- **`mutate(pre_sales_ymd = ...)` を使用して前回のデータを取得**
  - `lag(sales_ymd)` により、前回の売上日 `pre_sales_ymd` を取得します。
  - `lag(amount)` を使用し、前回の売上 `pre_amount` を取得します。

- **`mutate(diff_amount = ...)` を使用して売上の差分を計算**
...
`db_receipt` テーブル参照を使用して、日ごとの売上合計を計算し、前回との差分を求めます。
...
`receipt` テーブルを使用して、日ごとの売上合計を計算し、前回との差分を求めます。
