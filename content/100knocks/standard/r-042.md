...
1. `inner_join()` を使用して、**`df_sales_with_lag`** を **自身と結合 (自己結合)** します。  
   - 左側の `sales_ymd` に対して、右側の `sales_ymd.y` が `lag_ymd` 以上 `sales_ymd` 未満の範囲にあるレコードを結合します。
...
`df_receipt` を `db_receipt` に置き換えます。
...
`receipt` テーブルに基づいて、各売上日に対して 3 回前までの売上データを結合します。

#### `WITH` 句: `sales_data` CTE
...
1. **`sales_ymd` (売上日) 別の集計**  
   `SUM(amount)` により、各売上日 (`sales_ymd`) ごとに売上金額 (`amount`) の合計を集計します。
