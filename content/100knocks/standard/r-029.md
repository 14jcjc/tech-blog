...
slug: "r-029"
# draft: true
description: "店舗ごとに最も販売回数が多い商品を取得"
# summary: "店舗ごとに最も販売回数が多い商品を取得"
# weight: 10
categories: 
  - "100本ノック＋α (Lv1)"
tags: 
...
- `arrange(desc(n), store_cd)` により、`n` の降順で結果を並べ替え、同じ販売数の店舗については `store_cd` で昇順に並べます。
...
- `slice_max(n, n = 1, with_ties = TRUE, by = store_cd)` により、  
...
- **最後の `SELECT` 文**  
  - `n = max_n` の条件により、各店舗ごとに販売数が最大となる商品を抽出します。
  - `ORDER BY n DESC, store_cd` により、`n` の降順で結果を並べ替え、同じ販売数の店舗については `store_cd` で昇順に並べます。
...
- **CTE `product_rank`**  
  `RANK()` 関数を使用して、各店舗 (`store_cd`) ごとに商品を販売数 (`n`) の降順でランク付けします。

- **最後の `SELECT` 文**  
  `rank = 1` の条件により、各店舗で最も売れた商品を抽出します。
