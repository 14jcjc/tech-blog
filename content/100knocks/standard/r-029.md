...
`df_receipt` をテーブル参照 `db_receipt` に置き換えることで、同様の操作を実行できます。
...
- CTE **`product_num`** では、`receipt` テーブルから各店舗 (`store_cd`) における各商品 (`product_cd`) の販売数 (`n`) をカウントします。
- CTE **`product_max`** では、前の CTE からの結果を基に、  
  `MAX(n) OVER (PARTITION BY store_cd)` により、店舗ごとの最大販売数 (`max_n`) を計算します。  
  これにより、全ての商品の販売数とその店舗での最大販売数が結合されたデータセットが作成されます。
...
- CTE **`product_rank`** では、`RANK()` を用いて、各店舗 (`store_cd`) ごとに商品の販売数 (`n`) をランク付けします。
