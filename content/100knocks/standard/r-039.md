...
slug: "r-039"
# draft: true
description: "非会員を除外した売上上位顧客の完全外部結合"
# summary: ""
# weight: 10
categories: 
  - "100本ノック＋α (Lv1)"
...
##### `df_date` と `df_amount` の統合およびソート
   ...
   - `full_join()` を使用して、`df_date` と `df_amount` を `customer_id` 列で完全外部結合します。これにより、売上日数と売上金額の両方の情報を持つデータが得られます。  
   - 最後に、`arrange()` を使って、売上日数と合計金額の降順および `customer_id` の昇順でデータを並べ替えます。
...
- `purchase_data` から、`COUNT(DISTINCT sales_ymd)` により、顧客ごとのユニークな売上日をカウントします。
- `GROUP BY customer_id` で、顧客ごとに集計を行います。

##### CTE `ranked_purchase_dates`
...
- `customer_purchase_dates` に対し `RANK()` を使用して、売上日数 `n_date` に基づいてランク付けを行います。降順で並べ、売上日数が多い顧客に低いランク (1位〜) を付けます。
- 売上日数が同じ顧客には同じランクが付けられます。

##### CTE `customer_total_sales`
...
- `purchase_data` から顧客ごとの売上金額合計 (`SUM(amount)`) を計算します。
- `GROUP BY customer_id` で顧客ごとに集計します。

##### CTE `ranked_total_sales`
...
- `customer_total_sales` に対し `RANK()` を使用して、売上金額 `sum_amount` に基づいてランク付けを行います。売上金額が多い顧客に低いランク (1位〜) を付けます。
...
- 結果は、売上日数 (`n_date`) と売上金額 (`sum_amount`) の降順で並べ替えられ、さらに `customer_id` によって最終的に整理されます。
...
- **CTE `top_customers_by_dates`**  
  - 顧客ごとのユニークな売上日 (`n_date`) をカウントします。
  - `n_date` の降順で並べ替え、上位 20 人の顧客を `LIMIT 20` で抽出します。

- **CTE `top_customers_by_sales`**  
  - 顧客ごとの売上金額合計 (`sum_amount`) を計算します。
  - `sum_amount` の降順に並べ替え、上位 20 人の顧客を `LIMIT 20` で抽出します。
