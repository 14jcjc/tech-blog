...
##### `df_date` : 顧客ごとの売上日数の集計と上位 20 件の抽出
   ...
   - `df_rec` から各顧客ごとの売上日数（`n_date`）を計算します。`n_distinct(sales_ymd)` は、顧客ごとにユニークな売上日をカウントします。  
...
##### `df_amount` : 顧客ごとの総売上金額の集計と上位 20 件の抽出
...
- `customer_purchase_dates` に対し `RANK() OVER(...)` を使用して、売上日数 `n_date` に基づいてランク付けを行います。売上日数が多い顧客に低いランク (1位〜) を付けます。
...
- 結果は、売上日数 (`n_date`) と売上金額 (`sum_amount`) の降順で並べ替えられ、`customer_id` によって最終的に整理されます。
