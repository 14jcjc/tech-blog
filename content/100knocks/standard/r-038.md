...
3. **`summarise()`**
   ...
   - `na.rm = TRUE` で `NA` を無視して合計値を計算します。
...
- **`replace_na(list(sum_amount = 0.0))`**  
  `db_receipt` に存在しない `customer_id` については `sum_amount` が 0 ではなく欠損となるため、欠損値を `0.0` で補完します。
...
この SQL クエリは、`customer_id` ごとに `amount` の合計を計算し、欠損値を `0.0` に置き換えます。

- **`LEFT JOIN`** を使用して `customer` テーブルと `receipt` テーブルを結合します。
- **`WHERE`** 句で `gender_cd = '1'` (女性) かつ `customer_id` が `Z` で始まらない顧客を対象にします。
- **`GROUP BY`** で `customer_id` ごとに集約し、`SUM()` を適用します。
- **`COALESCE()`** を用いて、`NULL` を `0.0` に置き換えます。
- **`ORDER BY customer_id`** で `customer_id` の昇順に並べ、**`LIMIT 10`** で 10 件取得します。
